...
%token IF ELSE ADD SUB MUL DIV EQ LT LTE GT GTE NOT AND OR PREPOSTADD PREPOSTSUB LPAREN RPAREN LCURL RCURL SEMICOLON WHILE
...
StatementList:
		...
		| IF LPAREN RelExp2 RPAREN LCURL temp temp{
			printf("\nif %s goto %s:\ngoto %s:", $3, $6, $7);
			printf("\n%s:", $6);
		} StatementList { printf("\n%s:", $7); } RCURL ElseStmt { printf("\n%s:", genBlockLabel()); } StatementList { printf("\n"); }
                | WHILE LPAREN PreRelexp RPAREN LCURL begin temp {printf("\nif %s goto %s:\ngoto %s:", $3, $6, $7);}  { printf("\n%s:", $6); } StatementList
...
// need to store values via parent to get the things fixed in this i.e. remove the concatenation part. 
RelExp2:
         RelExp2 AND Relexp {
                strcat(str,genLabel);
                $$ = malloc(struct* bool);
                printf( "\nif %s goto %s", $1 ,  ); //this label is coming from parent
                printf ( "\ngoto %s" , str );
                //  generated label for condition 2 
                printf ( "\n %s :", str );

                printf( "\t if %s goto %s" , $3 , );
                printf ( "\ngoto %s", );
    	...
    	RelExp2 OR Relexp {
                strcat(str,genLabel);
                $$ = malloc(struct* bool);
                printf( "\nif %s goto %s", $1 , str ); //this label is coming from parent
                printf ( "\ngoto %s" , );
                //  generated label for condition 2 
                printf ( "\n %s :", str );
